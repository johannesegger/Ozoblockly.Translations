...
static string ApplyTranslations(string body)
{
    var replacements = new[] {
        ...
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_ADVANCED_TITLE=")4. Advanced(?=")"""), "4. Gewandter"),
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_BEGINNER_TITLE=")2. Beginner(?=")"""), "2. Anfänger"),
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_INTERMEDIATE_TITLE=")3. Intermediate(?=")"""), "3. Kompetenter"),
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_MASTER_TITLE=")5. Master(?=")"""), "5. Experte"),
