...
static string ApplyTranslations(string body)
{
    var replacements = new[] {
        ...
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_ADVANCED_TITLE=")4. Advanced(?=")"""), "4. Gewandte"),
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_BEGINNER_TITLE=")2. Beginner(?=")"""), "2. Anfängerin"),
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_INTERMEDIATE_TITLE=")3. Intermediate(?=")"""), "3. Kompetente"),
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_MASTER_TITLE=")5. Master(?=")"""), "5. Expertin"),
        (new Regex("""(?<=OZOBOT_HTML_LEVEL_NOVICE_TITLE=")1. Pre-Reader(?=")"""), "1. Neuling"),
